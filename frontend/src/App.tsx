...
export default function App() {
  ...
  async function handleDelete(id: string){
    try {
      ...
      const allCustomers = customers.filter( (costumer) => costumer.id !== id)
  ...
  return (
    <div className="w-full min-h-screen bg-gray-900 flex justify-center px-4">
      ...
      <form className="flex flex-col my-6" onSubmit={handleSubmit}>
        ...
        <input ref={emailRef} type="email" placeholder="Digite seu email..." className="w-full mb-5 p-2 rounded"/>
      ...
      <section className="flex flex-col gap-4">
        {customers.map( (customer) => (
                  <article key={customer.id} className="w-full bg-white rounded p-2 relative hover:scale-105 duration-300">
